...
class AssembledVectorDataAccessor:
    ...
    def create_interval_and_average_vectors_df(
        ...
        `Input:`
        * sampling_frequency: str - Sampling frequency for inteval delta and average rate calculation
    ...
    def create_vector_calculator_vectors_df(self) -> pd.DataFrame:
        if not self.has_vector_calculator_vectors():
            raise ValueError(
                f'Vector data handler for provider "{self._name}" has no vector calculator '
                "expressions"
            )
