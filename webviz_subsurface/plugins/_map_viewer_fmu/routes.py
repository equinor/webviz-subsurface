...
from .utils.surface_utils import surface_spec_from_url, surface_to_rgba


def deckgl_map_routes(app: Dash, surface_set_models: List[SurfaceSetModel]) -> None:
    @CACHE.memoize(timeout=CACHE.TIMEOUT)
    def _send_surface_as_png(hash: str):
        surface_spec = surface_spec_from_url(hash)
        ensemble = surface_spec.ensemble
        surface = surface_set_models[ensemble].get_surface(surface_spec)
