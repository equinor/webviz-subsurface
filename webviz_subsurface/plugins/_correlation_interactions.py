...
class CorrelationInteractions(WebvizPluginABC):
    ...
    def set_callbacks(self, app):
        ...
        def _update_table(ensemble, response, *filters):
            ...
            X = make_interaction_df(parameterdf)
            y = df[response]

            table = correlation_interactions(X, y)
            table.index.name = "Parameter interaction terms"
            ...
            return(
                    data,
                    columns,
                    f"Correlation between {response} and parameter interaction terms"
                )
...
def make_interaction_df(parameterdf):
    cols = [
    ...
    # The number in the dict is the depth / number of products
    combos = {1: cols}
    depth=3 

    #Get combinations
    for n in range(2,depth+1):   
        combos[n]=list(itertools.combinations(cols,n))

    #Create a DataFrame of parameters including the new products
    Xc = X.copy() # Taking a copy to avoid errors of setting values at a slice of a dataframe
    for i in range(2,depth+1):
        for tupl in combos[i]:
            temp = Xc[tupl[0]]
            for j in range(1,i):
                temp = temp*Xc[tupl[j]]
            Xc[" * ".join(tupl)] = temp
...
def correlation_interactions(X, y):
    df = pd.DataFrame(X.corrwith(y, method=lambda x, y: pearsonr(x,y)))
    merged_df = pd.DataFrame()
    for col in df.columns:
        feature_columns = [
            "Correlation coefficient".format(col=col), 
            "P-value".format(col=col)
        ]
        merged_df[feature_columns] = df[col].apply(lambda s: pd.Series({ feature_columns[0]: s[0],
                                                                    feature_columns[1]: s[1]} ) )

    sort_by_pval_df = merged_df.sort_values(by=["P-value"]).head(20)

    return sort_by_pval_df
