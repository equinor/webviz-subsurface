...
# fmt: off
TEST_INPUT_COLUMNS = ["DATE", "REAL",  "A", "B"]
TEST_INPUT_DATA = [
...
def test_create_vectors_statistics_df() -> None:
    ...
    df.columns = TEST_INPUT_COLUMNS
    statistics_df = create_vectors_statistics_df(df)

    expected_df = pd.DataFrame(TEST_EXPECTED_DATA)
    expected_df.columns = pd.MultiIndex.from_tuples(TEST_EXPECTED_COLUMNS)

    assert statistics_df.equals(expected_df)
    assert list(statistics_df.columns) == list(expected_df.columns)
